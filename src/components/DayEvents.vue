            ...
            <div v-if="hasChecklist(event)" class="event-checklist">
              ...
              >
                <input
                  ...
                  type="checkbox"
                  @change.stop="
                    toggleChecklistItem(event, item.id, $event.target.checked)
                  "
                />
                <span :class="{ 'event-checklist-item--done': item.done }">{{
                  item.text
                }}</span>
              </label>
            </div>
        ...
        <v-card-text class="d-flex flex-column ga-3">
          ...
          <div class="d-flex align-center justify-space-between">
            ...
            <v-btn
              ...
              size="x-small"
              variant="text"
              @click="addDraftChecklistItem"
              >Добавить пункт</v-btn
            >
          </div>
          <div
            v-if="(draftEvent.checklist || []).length > 0"
            class="d-flex flex-column ga-2"
          >
            <div
              v-for="item in draftEvent.checklist"
              :key="item.id"
            ...
            >
              ...
              <v-btn
                ...
                size="x-small"
                variant="text"
                @click="removeDraftChecklistItem(item.id)"
              />
            </div>
          </div>
          <v-text-field
...
import {
  ...
  getCategoryById,
} from "@/modules/categories/categoryState";
import { addActionLog } from "@/modules/timers/timerState";
import DaySketchPad from "./DaySketchPad.vue";
...
export default {
    ...
    events: {
      ...
      handler(value) {
        ...
        this.localEvents = value.map((item) => ({
          ...item,
          checklist: this.normalizeChecklist(item.checklist),
  ...
  methods: {
    ...
    updateSketchMetrics(el) {
      ...
      this.sketchTopOffset = stageRect
        ? Math.max(0, scrollRect.top - stageRect.top)
        : el.offsetTop;
    },
    ...
    addDraftChecklistItem() {
      if (!Array.isArray(this.draftEvent.checklist))
        this.draftEvent.checklist = [];
      this.draftEvent.checklist.push({
        id: crypto.randomUUID(),
        text: "",
        done: false,
      });
    ...
    removeDraftChecklistItem(itemId) {
      this.draftEvent.checklist = (this.draftEvent.checklist || []).filter(
        (item) => item.id !== itemId,
      );
    },
    toggleChecklistItem(event, itemId, done) {
      ...
      const updatedChecklist = this.normalizeChecklist(
        this.localEvents[idx].checklist,
      ).map((item) =>
