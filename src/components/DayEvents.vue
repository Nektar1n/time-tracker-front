    ...
    methods: {
      ...
      saveEvent () {
        ...
        if (idx !== -1) {
          ...
          const parsedStart = this.parseDateTimeInput(
            this.draftStartInput,
            updatedEvent.start,
          )
          const parsedEnd = this.parseDateTimeInput(
            this.draftEndInput,
            updatedEvent.end,
          )

          updatedEvent.start = parsedStart
          updatedEvent.end = Math.max(parsedStart, parsedEnd)
      ...
      startDrag (nativeEvent, { event, timed }) {
        if (!nativeEvent.target?.closest('.event-drag-handle')) {
          return
        }
      ...
      parseDateTimeInput (value, fallback) {
        ...
        return this.toTimestamp(fallback)
      },
      getEventColor (event) {
        return event.color
      },
